...
$('.btn-gerar-fatura').click(() => {
    ...
    $('.lbl-total_fatura').text($('#painel-total-venda').text())
})
...
$(document).on('change', '.check-conta', function () {
    const total = $('.check-conta:checked').length;
    ...
    btn.html(
        total > 0
        ? `<i class="ri-cash-line me-1"></i> Receber selecionadas (${total})`
...
$('.efetuar_pagamento').click(() => {
    ...
    $('#painel-total-venda').text(convertFloatToMoeda(total))
