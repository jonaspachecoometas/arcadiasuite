...
<body>
    ...
    <table>
        ...
        <tbody>
            ...
            @if(sizeof($i->rastroXml) > 0)
            @foreach($i->rastroXml as $r)
            <tr>
                <th colspan="10" style="text-align: left">
                    <label>Nº Lote <strong style="color: #777">{{ $r['nLote'] }}</strong>,</label>
                    <label>Qtd. Lote <strong style="color: #777">{{ $r['qLote'] }}</strong>,</label>
                    <label>Data Fabricação <strong style="color: #777">{{ __data_pt($r['dFab'], 0) }}</strong>,</label>
                    <label>Data Validade <strong style="color: #777">{{ __data_pt($r['dVal'], 0) }}</strong></label>
                </th>
            </tr>
            @endforeach
            @endif
